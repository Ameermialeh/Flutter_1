...
class _ChatInputFieldState extends State<ChatInputField> {
  ...
  void _sendMessage() async {
    ...
    if (enteredMessage.trim().isNotEmpty && _pickedImageFile.isEmpty) {
      ...
      addDocument('users', widget.userID, widget.id);
      addDocument('services', widget.id, widget.userID);
      //
  ...
  Future<void> addDocument(String user, String doc1, String doc2) async {
    try {
      FirebaseFirestore firestore = FirebaseFirestore.instance;

      CollectionReference<Map<String, dynamic>> collectionReference =
          firestore.collection(user).doc(doc1).collection('chatUsers');

      await collectionReference.doc(doc2).set({});
    } catch (error) {
      print('Error adding empty document: $error');
    }
  }
}
